...
public class EntityFactory extends Factory {
	...
	public EntityFactory(ExtendedWorld world, Stage stage) {
		super(world, stage);
		actorMapper = ComponentMapper.getFor(ActorComponent.class, world);
	}
	...
	public Entity createTile(float x, float y, int type) {
		...
		PhysicsModel square = new PhysicsModel(world.box2dWorld, e, "models.json", model, 0.0f, 0.0f, 0.5f,
				GameBits.SCENERY, false, 1.0f);
	...
	public Entity createChlotz(float x, float y) {
		...
		PhysicsModel square = new PhysicsModel(world.box2dWorld, e, "models.json", "Square1", 1.0f, 1.0f, 0.5f,
				GameBits.SCENERY, false, 1.0f);
		PhysicsComponent pc = get(PhysicsComponent.class).init(square.getBody());
	...
	public Entity createShip(float x, float y, Bits bits) {
		...
		PhysicsModel shipModel = new PhysicsModel(world.box2dWorld, e, "models.json", "Ship1", 2.0f, 1.0f, 0.0f, bits,
				false, 1.0f);
	...
	public Entity createProjectile(float x, float y, Bits bits, Vector2 velocity) {
		...
		PhysicsModel projectile = new PhysicsModel(world.box2dWorld, e, "models.json", "Projectile", 0.3f, 1.0f, 0.0f,
				bits, false, 0.5f);
